...
app.use(express.static(path.join(__dirname, '../fbo-client/build')));
...
app.get('/api/getPdfInv', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/getInvdata', authenticateToken, async (req, res) => {
    try {
        // Kết nối đến cơ sở dữ liệu
        await sql.connect(config);
...
app.get('*', (req, res) => {
    res.sendFile(path.join(__dirname, '../fbo-client/build', 'index.html'));
});
...
function generateToken(user) {
    ...
    const options = {
        expiresIn: '2h', // Token sẽ hết hạn sau 24 giờ
    };
